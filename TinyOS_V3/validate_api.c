...
BOOT_TEST(test_mainthread_threadexit_secondary_exit,
	"Test that multiple secondary threads can Exit and the primary can ThreadExit."
	)
{
	...
	int mthread(int argl, void* args){
		for(int i=0;i<5;i++){
			Tid_t tid;
			ASSERT((tid = CreateThread(task, i*5+20, NULL)) != NOTHREAD);
			ASSERT(ThreadDetach(tid) == 0);
		}
		fibo(35);
		ThreadExit(5);
		return 0;
	...
	Exec(mthread, 0, NULL);
	int exitval;
	ASSERT(WaitChild(NOPROC, &exitval)!=NOPROC);
	ASSERT(exitval == fibo(20));
...
BOOT_TEST(test_exit_detached_threads,
	"Test that a process thread calling Exit will clean up correctly the detached threads."
	)
{
...
BOOT_TEST(test_detach_a_joined_thread,
	"Test that a process thread detaching another will fail to join."
	)
{
	...
	int mthread(int argl, void* args){
		Tid_t tid;
		ASSERT((tid = CreateThread(task, 0, NULL)) != NOTHREAD);
		ASSERT(ThreadJoin(tid, NULL) == -1);
		fibo(40);
		return 0;
	}
	...
	)
{
	...
	&test_exit_detached_threads,
	&test_detach_a_joined_thread,
	&test_mainthread_threadexit_secondary_exit,
	NULL
};
