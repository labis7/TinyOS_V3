...
//void Better_Mutex_Lock(Mutex*, TCB* owner){
...
int Cond_Wait_Flag(Mutex* mutex, CondVar* cv, Yield_Reason YR)
{
  ...
  Mutex_Unlock(mutex);
  sleep_releasing(STOPPED, &(cv->waitset_lock), YR);
...
{
 return Cond_Wait_Flag(mutex, cv, SR);
}
...
static __cv_waitset_node* cv_signal(CondVar* cv)
{
  ...
  if(cv->waitset != NULL) {
	...
	while(node != NULL && ((TCB*)node->thread)->owner_ptcb->interrupted == 1 ){
		node = node->next;
	}
	if(node != NULL){
