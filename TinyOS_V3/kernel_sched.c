...
TCB* spawn_thread(PCB* pcb, void (*func)())
{
  ...
  tcb->priority = MAX_PRIORITY_QUEUES/2;
  tcb->mx_lock_fails = 0;
  tcb->pbackup = MAX_PRIORITY_QUEUES/2;
...
rlnode SCHED[MAX_PRIORITY_QUEUES];                         /* The scheduler queue */
...
void sched_queue_add(TCB* tcb)
{
  /* Insert at the end of the scheduling list */
	...
	tcb->curtimemicro = curtime.tv_sec*1000000 + curtime.tv_nsec/1000;
...
TCB* sched_queue_select()
{
	...
	struct timespec curtime;
	CHECK(clock_gettime(CLOCK_REALTIME, &curtime));
	unsigned long time_now= curtime.tv_sec*1000000 + curtime.tv_nsec/1000;
	for(int i=0;i<=MAX_PRIORITY_QUEUES-2;i++){
		if(is_rlist_empty(SCHED+i+1))continue;
		tmp=(SCHED+i+1)->next;                //we use that tmp in order to move through the list, we choose the second one for init.
		while(tmp!=SCHED+i+1){		
			tmp2=tmp->next;
			if((time_now -(tmp->tcb->curtimemicro) > QUANTUMS_FREEZED*QUANTUM)){	
				tmp->tcb->priority=(tmp->tcb->priority) - 1 ;	
				rlist_push_back(&SCHED[tmp->tcb->priority],rlist_remove(&(tmp->tcb->sched_node))); //to teleut. sthn parenthesi tokana gia test se ena warning
			}
			tmp=tmp2;
		}
	}
 ...
 */
void sleep_releasing(Thread_state state, Mutex* mx, Yield_Reason YR)
{
  ...
  if(state != STOPPED || tcb->owner_ptcb->interrupted == 0){
...
void yield(Yield_Reason YR)
{ 
  double RQ = bios_cancel_timer();/**< Remaining Quantum*/
  ...
  double percentage = 1.0 - RQ/((double)QUANTUM);
  //if(percentage>0.0 && percentage < 0.5)
//	  fprintf(stderr,"edw malakaaaaaaaaaaaaaaaaaaa %f\n", percentage);
  Mutex_Lock(& current->state_spinlock);
  if(YR!=ML){
	  current->priority = current->pbackup;
  }
  //fprintf(stderr, "%d, %d\n", YR, CURTHREAD->priority);
  switch(YR)
  {
    case QR:
//	    fprintf(stderr, "QR: %f\n", percentage);
      if(current->priority < MAX_PRIORITY_QUEUES - 1)
	      ++(current->priority);
      current->mx_lock_fails = 0;
      break;
    case IO:
      //fprintf(stderr,"IO: %f\n", percentage);
      if(percentage < 0.25){
    ...
    case ML://10% 20% 30% 40%
      //fprintf(stderr, "ML: %f\n", percentage);
       ++(current->mx_lock_fails);
 	if(current->mx_lock_fails>=2){
		if(current->priority < MAX_PRIORITY_QUEUES - 1){
			current->priority += 1;
		}else{
			current->priority = MAX_PRIORITY_QUEUES-1;
		}
	}
	break;
    case SR:
	...
	break;
    default:
	fprintf(stderr,"Uknown Yield Reason");
	break;
  }
  if(YR!=ML){
	  current->pbackup = current->priority;
  }
...
{
  //for(int i=0;i<MAX_BETTER_MUTEXES;i++){
    //bmx[i] = BETTER_MUTEX_INIT;
  //}
  for(int i=0;i<MAX_PRIORITY_QUEUES;i++){
